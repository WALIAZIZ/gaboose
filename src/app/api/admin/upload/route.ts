...
export async function POST(request: NextRequest) {
  try {
    ...
    if (file.size > MAX_INPUT_SIZE) return NextResponse.json({ error: 'File too large' }, { status: 400 })
    ...
    return NextResponse.json({ url: dataUrl, meta: { originalSize: (inputBuffer.length / 1024).toFixed(0) + 'KB', compressedSize: (outputBuffer.length / 1024).toFixed(0) + 'KB', saved: Math.round((1 - outputBuffer.length / inputBuffer.length) * 100) + '%' } })
  } catch (error: any) {
