...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) return NextResponse.json({ error: `Invalid file type: ${file.type}. Use JPEG, PNG, WebP, or GIF.` }, { status: 400 })
    if (file.size === 0) return NextResponse.json({ error: 'File is empty' }, { status: 400 })
    if (file.size > MAX_INPUT_SIZE) return NextResponse.json({ error: `File too large: ${(file.size / 1024 / 1024).toFixed(1)}MB. Max is 5MB.` }, { status: 400 })
