  ...
  async function handleAdd() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDialogOpen(false)
        setNewImage({ title: '', category: 'gallery', imageUrl: '', sortOrder: '0' })
        fetchImages()
      }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={fetchImages} style={{ borderColor: '#1E1E24', color: '#A09890' }}>
            <RefreshCw className="w-4 h-4 mr-2" /> Refresh
          </Button>
          <Button size="sm" onClick={() => setDialogOpen(true)} style={{ backgroundColor: '#C4A03C', color: '#000' }}>
      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {[...Array(8)].map((_, i) => <Skeleton key={i} className="h-56 rounded-xl" style={{ backgroundColor: '#111114' }} />)}
        </div>
      ) : images.length === 0 ? (
            ...
            <Card key={img.id} className="group overflow-hidden" style={cardStyle}>
              <div className="relative aspect-square overflow-hidden" style={{ backgroundColor: '#08080A' }}>
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center">
                  <Button size="sm" variant="ghost" className="h-9 w-9 p-0 bg-red-600/80 hover:bg-red-600 opacity-0 group-hover:opacity-100 transition-opacity"
              ...
              <CardContent className="p-3">
                ...
                <Select value={img.category} onValueChange={(v) => updateCategory(img.id, v)}>
                  <SelectTrigger className="h-7 mt-2 text-xs" style={{ backgroundColor: '#08080A', borderColor: '#1E1E24', color: '#A09890' }}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent style={{ backgroundColor: '#111114', borderColor: '#1E1E24' }}>
                    {categories.map((c) => <SelectItem key={c} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</SelectItem>)}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent style={{ backgroundColor: '#111114', borderColor: '#1E1E24' }}>
          <DialogHeader>
            <DialogTitle style={{ color: '#fff' }}>Add New Image</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input value={newImage.title} onChange={(e) => setNewImage({ ...newImage, title: e.target.value })}
                placeholder="Image title" style={{ backgroundColor: '#08080A', borderColor: '#1E1E24', color: '#fff' }} />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              {newImage.imageUrl ? (
                <div className="relative w-full h-48 rounded-lg overflow-hidden">
                  ...
                  <button onClick={() => setNewImage({ ...newImage, imageUrl: '' })}
                    className="absolute top-2 right-2 w-7 h-7 bg-black/60 rounded-full flex items-center justify-center">
                    <X className="w-4 h-4 text-white" />
                  </button>
                </div>
              ) : (
                <label className="flex flex-col items-center justify-center w-full h-40 rounded-lg border-2 border-dashed cursor-pointer"
                  style={{ borderColor: '#1E1E24', backgroundColor: '#08080A' }}>
                  <input type="file" accept="image/*" onChange={handleUpload} className="hidden" />
                  {uploading ? (
                    <div className="animate-spin w-6 h-6 border-2 border-t-transparent rounded-full" style={{ borderColor: '#C4A03C', borderTopColor: 'transparent' }} />
                  ) : (
                    <>
                      <Upload className="w-8 h-8 mb-2" style={{ color: '#A09890' }} />
                      <span className="text-sm" style={{ color: '#A09890' }}>Click to upload (max 5MB)</span>
                    </>
                  )}
                </label>
              )}
            ...
            <div className="flex gap-3 pt-2">
              <Button onClick={handleAdd} className="flex-1" style={{ backgroundColor: '#C4A03C', color: '#000' }}>
                Add Image
              </Button>
